...
class PostTweetActivity(ActivityBase):
    ...
    async def _mint_nft_with_crossmint(self, image_url: str, tweet_text: str, personality_data: Dict[str, Any]) -> Dict:
        """
        Mint an NFT using the Crossmint API
        """
        import aiohttp
        from datetime import datetime

        API_KEY = await api_manager.get_api_key("crossmint", "CROSSMINT")
        CHAIN = "story-mainnet"
        ENV = "www"
        ...
        # Ensure personality_data is a dict and not None
        personality_data = personality_data or {}
        
        # More strict filtering of personality data
        filtered_personality = {}
        for k, v in personality_data.items():
            if k is not None and v is not None and str(k).strip() and str(v).strip():
                try:
                    filtered_personality[str(k).strip()] = str(v).strip()
                except (ValueError, TypeError):
                    continue
        
        # Create personality string only if we have filtered data
        personality_str = (
            ", ".join(f"{k}: {v}" for k, v in filtered_personality.items())
            if filtered_personality
            else "No personality data available"
        )
        
        # Ensure all metadata values are strings and not None
        metadata = {
            "name": f"AI Generated Tweet NFT - {datetime.now().strftime('%Y-%m-%d %H:%M')}",
            "image": str(image_url or ""),
            "description": f"Tweet: {str(tweet_text or '')}\n\nGenerated by AI with personality: {personality_str}",
        }

        payload = {
            "recipient": f"email:{RECIPIENT_EMAIL}:{CHAIN}",
            "metadata": metadata
        }
